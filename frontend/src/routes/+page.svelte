<script>
    ...
    async function fetchHighScores() {
        try {
            ...
            console.log('High scores:', highScores);  // Tambahkan ini
            } else {
  ...
  <!-- Template HTML tidak berubah, tetap sama seperti sebelumnya -->
      ...
      {:else if gameStarted}
        <div in:fly="{{ y: 50, duration: 500 }}">
          ...
          <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
            <div class="bg-indigo-600 h-2.5 rounded-full transition-all duration-500" style="width: {(timeLeft / 60) * 100}%"></div>
          </div>
        </div>
      {:else if gameEnded}
        <div in:fade="{{ duration: 500 }}">
